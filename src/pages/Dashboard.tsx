...
import { useGetUserWorkspacesQuery, type IWorkspace } from '../app/api/workspace';
...
export function Dashboard() {
  ...
  const workspaces: IWorkspace[] = Array.isArray(response)
    ? response
    : (response as { data?: { workspaces?: IWorkspace[] } } | undefined)?.data?.workspaces || [];
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-lg text-slate-400">
          Welcome to your dashboard! Manage your workspaces, view your projects, and keep track of your team's progress.
        </p>
      </div>
      ...
      <div className="mt-12">
        <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
          ...
          <div className="flex-1 w-full">
            <h2 className="text-2xl font-semibold text-white mb-4">Your Workspaces</h2>

            {/* 1. Loading State */}
            {isLoading && (
              <div className="flex items-center justify-center p-12 bg-[#13161c] border border-[#1e232d] rounded-2xl">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-500" />
                <span className="ml-3 text-slate-400 text-sm">Loading workspaces...</span>
              </div>
            )}
            ...
            {isError && (
              <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl text-sm">
                Failed to load workspaces. Open DevTools Console (F12) to inspect the error payload.
              </div>
            )}
                ...
                {workspaces.map((workspace) => {
                  ...
                  return (
                    <WorkspaceCard
                      ...
                      onOpen={() => navigate(`/home/boards/${workspace._id}`)}
                      onAddBoard={() => navigate(`/home/boards/${workspace._id}`)}
                    />
                  );
                })}
              </div>
